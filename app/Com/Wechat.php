...
class Wechat {
	 ...
	 */
	public function response($content, $nonce, $type = 'text', $flag = 0){
		/* 基础数据 */
		$this->data = array(
	 ...
	 * @param  array $cert_path  证书路径
	 * @return array   $data   响应数据
	 */
	private function http($url, $params = array(), $method = 'GET', $ssl = false , $cert_path = null){
		...
		if ($ssl) {
			$pemPath = dirname(__FILE__).'/Weixinpay/';
			$pemCret = '/home/wwwroot/www.jxjykeji.com/ThinkPHP/Library/Vendor/Weixinpay/wapcert/apiclient_cert.pem';
			$pemKey  = '/home/wwwroot/www.jxjykeji.com/ThinkPHP/Library/Vendor/Weixinpay/wapcert/apiclient_key.pem';
			if($cert_path != null){
				$pemCret = $cert_path['cert_path'];
				$pemKey =  $cert_path['key_path'];
			}
			if (!file_exists($pemCret)) {
				exit(return_json('error','证书不存在',false));
//				$this->error = '证书不存在';
//				return false;
			}
			if (!file_exists($pemKey)) {
				exit(return_json('error','密钥不存在',false));
//				$this->error = '密钥不存在';
//				return false;
			}
	...
	public function material_news($articles) {
		$url = self::MATERIAL_NEWS_URL . '?access_token=' . $this->access_token;
	 ...
	 * @param strng $order_on 订单号
	 * @param  string $check_name 真实姓名校验
	 *         NO_CHECK：不校验真实姓名
	 *         FORCE_CHECK：强校验真实姓名（未实名认证的用户会校验失败，无法转账）
	 *         OPTION_CHECK：针对已实名认证的用户才校验真实姓名（未实名认证用户不校验，可以转账成功） 
	 * @param  string $desc 描述
	 * @param  array $DisPath 证书路径
	 * @return boolean|array
	 */
	public function transfers($openid, $amount, $desc , $order_on , $check_name = 'NO_CHECK' ,$DisPath) {
		...
		if(empty($order_on)){
			return '{"code":"error","msg":"订单号错误","login_state":"0"}';
		}
		$params['amount']           = $amount * 100;
		$params['desc']             = $desc;
		$params['spbill_create_ip'] = get_client_ip();
		$params['partner_trade_no'] = $order_on;
		$params['mch_appid']        = $this->appid;
		$params['mchid']            = $this->mch_id;
		$params['nonce_str']        = self::_getRandomStr();
		$params['sign']             = self::_getOrderMd5($params);
		$data = self::_array2Xml($params);
		$data = $this->http(self::PAY_TRANSFERS_URL, $data, 'POST', true , $DisPath);
		return self::parsePayRequest($data);
	}
